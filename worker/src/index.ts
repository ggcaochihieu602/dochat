    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: "Đây là ảnh chụp tài liệu tiếng Việt. Hãy trích xuất TOÀN BỘ văn bản từ ảnh này. Xuất ra DUY NHẤT phần văn bản đã trích xuất, giữ nguyên thứ tự đọc (trên xuống, trái qua phải), không giải thích, không bình luận.",
          },
        ],
      },
    ],
...
async function handleMessage(update: any, env: Env): Promise<void> {
  ...
  const job: Job = {
    ...
    chat_id: chatId,
    user_id: userId,
    source_type: finalSource.source_type,
    mode: state.mode,
    file_id: finalSource.file_id,
